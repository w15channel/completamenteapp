export default async function handler(req, res) {
  ...
  try {
    // 3. Comunicação: Envia a requisição para o Hugging Face escondendo a chave do usuário
    ...
    if (!response.ok) {
      ...
      console.error('Falha na resposta do Hugging Face:', errorData);
    ...
    return res.status(500).json({ error: 'Erro interno ao processar a resposta do Hugging Face.' });
  }
}
